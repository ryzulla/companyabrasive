...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6">
        ...
        @endif
        <form method="POST" action="{{ route('admin.blogs.store') }}" enctype="multipart/form-data" class="space-y-5" id="blogForm">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="meta" value="{{ old('meta') }}" required
                       class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            <div>
                ...
                <textarea name="desc" id="descInput" class="hidden">{{ old('desc') }}</textarea>
                {{-- CKEditor container --}}
                <div id="editor" class="border border-slate-200 rounded-lg overflow-hidden">{{ old('desc') }}</div>
                @error('desc')
