    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <form method="POST" action="{{ route('admin.faqs.update', $faq) }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="question" rows="2" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('question', $faq->question) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-700 mb-1.5">Jawaban <span class="text-red-500">*</span></label>
                <textarea name="answer" rows="4" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none">{{ old('answer', $faq->answer) }}</textarea>
            </div>
