...
@section('content')
@php $v = fn($key, $def='') => $settings[$key] ?? $def; @endphp
...
<form id="settingsForm" method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data">
    ...
    <div id="panel-seksi" class="settings-panel" style="display:none;">
        <div style="background:#fff; border:1px solid #e5e7eb; border-radius:8px; padding:24px;">
            <h3 style="font-size:15px; font-weight:600; color:#1f2937; margin:0 0 20px; padding-bottom:12px; border-bottom:1px solid #f3f4f6;">Judul &amp; Deskripsi Tiap Seksi</h3>
            <div style="display:flex; flex-direction:column; gap:12px;">
                @php
                    $sections = [
                        ['categories_title', 'categories_desc', 'Kategori'],
                        ['products_title',   'products_desc',   'Produk'],
                        ['video_title',      'video_desc',      'Video'],
                        ['clients_title',    'clients_desc',    'Klien / Mitra'],
                        ['faq_title',        'faq_desc',        'FAQ'],
                        ['blog_title',       'blog_desc',       'Blog'],
                        ['testimonials_title', null,            'Testimoni'],
                    ];
                @endphp
                @foreach($sections as [$titleKey, $descKey, $sectionName])
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; padding-bottom:12px; border-bottom:1px solid #f9fafb;">
                        <div>
                            <label class="field-label">{{ $sectionName }} — Judul</label>
                            <input type="text" name="{{ $titleKey }}" value="{{ $v($titleKey) }}" class="field-input">
                        </div>
                        @if($descKey)
                            <div>
                                <label class="field-label">{{ $sectionName }} — Deskripsi</label>
