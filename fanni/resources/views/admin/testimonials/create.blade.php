    ...
    <div class="bg-white rounded-xl border border-slate-100 p-6">
        ...
        <form method="POST" action="{{ route('admin.testimonials.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="text" rows="4" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none" placeholder="Cerita pengalaman pelanggan...">{{ old('text') }}</textarea>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Nama <span class="text-red-500">*</span></label>
                    <input type="text" name="author" value="{{ old('author') }}" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Nama pelanggan">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Jabatan / Perusahaan <span class="text-red-500">*</span></label>
                    <input type="text" name="pos" value="{{ old('pos') }}" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Manager, PT. ABC">
                </div>
            </div>
